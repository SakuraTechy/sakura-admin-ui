  ...
  >
    ...
    >
      ...
      <a-alert type="info" show-icon class="recording-tip">
        请确保已安装并启用 CueCast Chrome 扩展，当前使用浏览器录制的方式，采集页面元素信息，包含原始定位（元素属性，文本，截图，鼠标事件）等。
      </a-alert>
              ...
              >
                ...
                >
                  <div class="option-row">
                    <span class="option-main">{{ item.serverIp }}</span>
                    <a-tag size="small" :color="item.statusColor">{{ item.statusLabel }}</a-tag>
                  </div>
                </a-option>
              </a-select>
            </a-form-item>
      ...
      >
        ...
        <div class="target-scope-content">
          ...
          <a-descriptions v-if="isTargetSceneFixed" :column="2" bordered size="small" class="target-summary">
            <a-descriptions-item label="目标场景">
              <a-tooltip :content="targetScene?.sceneId || '-'">
                <span class="target-summary-value">{{ targetScene?.sceneId || '-' }}</span>
              </a-tooltip>
            </a-descriptions-item>
            <a-descriptions-item label="场景名称">
              <a-tooltip :content="targetScene?.name || '-'">
                <span class="target-summary-value">{{ targetScene?.name || '-' }}</span>
              </a-tooltip>
            </a-descriptions-item>
          </a-descriptions>
          <a-row
            v-if="(targetCaseModes.includes(form.mode) && !isTargetCaseFixed) || (form.mode === 'replaceStep' && !isTargetStepFixed)"
            :gutter="16"
            class="target-scope-select-row"
          >
          ...
          <a-descriptions v-if="isTargetCaseFixed" :column="2" bordered size="small" class="target-summary">
            <a-descriptions-item label="目标用例">
              <a-tooltip :content="form.targetCaseId || '-'">
                <span class="target-summary-value">{{ form.targetCaseId || '-' }}</span>
              </a-tooltip>
            </a-descriptions-item>
            <a-descriptions-item label="用例名称">
              <a-tooltip :content="selectedTargetCase?.name || '-'">
                <span class="target-summary-value">{{ selectedTargetCase?.name || '-' }}</span>
              </a-tooltip>
            </a-descriptions-item>
          </a-descriptions>
          <a-descriptions v-if="isTargetStepFixed" :column="2" bordered size="small" class="target-summary">
            <a-descriptions-item label="目标步骤">
              <a-tooltip :content="form.targetStepId || '-'">
                <span class="target-summary-value">{{ form.targetStepId || '-' }}</span>
              </a-tooltip>
            </a-descriptions-item>
            <a-descriptions-item label="步骤名称">
              <a-tooltip :content="selectedTargetStep?.label || '-'">
                <span class="target-summary-value">{{ selectedTargetStep?.label || '-' }}</span>
              </a-tooltip>
            </a-descriptions-item>
          </a-descriptions>
        </div>
      ...
      >
        <span v-if="viewMode === 'cardSidebar' && advancedSideCollapsed" class="advanced-side-toggle-anchor">
        ...
        <div v-if="viewMode === 'cardSidebar'" class="advanced-side-toolbar">
          ...
          <span v-if="!advancedSideCollapsed" class="advanced-side-toggle-inline">
            <a-tooltip content="收起高级选项">
              <a-button
                class="advanced-side-toggle"
                size="mini"
                shape="circle"
                @click="advancedSideCollapsed = true"
              >
                <template #icon>
                  <icon-left />
                </template>
...
const getEnvironmentStatusMeta = (status: unknown) => {
  const isOnline = String(status) === '5' || status === '在线'
  return {
...
const refreshProjectEnvironmentStatuses = async () => {
  await Promise.all(projectEnvironmentOptions.value.map(async (option) => {
    try {
      const { data } = await getProjectEnvironmentRuntimeStatus(option.value)
      Object.assign(option, {
        ...getEnvironmentStatusMeta(data?.onlineStatus),
        serverIp: data?.serverIp || option.serverIp,
      })
    } catch {
      // 实时状态查询失败时保留配置接口返回的状态，不阻断录制环境选择。
    }
  }))
}
...
const loadProjectEnvironments = async () => {
  ...
  try {
    ...
    projectEnvironmentOptions.value = data.map((item: any) => {
      ...
      const statusMeta = getEnvironmentStatusMeta(server?.status ?? item?.status)
      return {
        value: String(item.id),
    ...
    await refreshProjectEnvironmentStatuses()
  } finally {
...
:deep(.target-scope-content > .arco-form-item),
:deep(.target-scope-select-row .arco-form-item) {
  margin-bottom: 0;
}

.target-scope-select-row {
  margin-top: 0;
  margin-bottom: 0;
}
...
.target-summary {
  margin-bottom: 0;
  --target-summary-label-width: 96px;
  --target-summary-left-value-width: 210px;
  --target-summary-right-value-max-width: 560px;
}
...
:deep(.target-summary .arco-descriptions-table) {
  width: 100%;
  table-layout: fixed;
}

:deep(.target-summary .arco-descriptions-item-label) {
  width: var(--target-summary-label-width);
  max-width: var(--target-summary-label-width);
  white-space: nowrap;
}

:deep(.target-summary .arco-descriptions-item-value) {
  min-width: 0;
}

:deep(.target-summary .arco-descriptions-row > .arco-descriptions-item-value:nth-child(2)) {
  width: var(--target-summary-left-value-width);
  max-width: var(--target-summary-left-value-width);
}

:deep(.target-summary .arco-descriptions-row > .arco-descriptions-item-value:nth-child(4)) {
  width: auto;
  max-width: var(--target-summary-right-value-max-width);
}

:deep(.target-summary .arco-trigger) {
  display: block;
  width: 100%;
  min-width: 0;
  overflow: hidden;
...
.recording-view--cardSidebar {
  ...
  :deep(.advanced-settings .arco-collapse-item),
  :deep(.advanced-settings .arco-collapse-item-content),
  :deep(.advanced-settings .arco-collapse-item-content-box) {
    display: flex;
    flex-direction: column;
  ...
  .recording-option-group :deep(.arco-radio-group) {
    display: flex;
    flex-wrap: wrap;
  ...
  .recording-option-group :deep(.arco-radio-group-direction-vertical) {
    flex-direction: column;
    align-items: flex-start;
    gap: clamp(8px, 0.8vh, 8px);
  }

  .recording-option-group :deep(.arco-radio-label) {
    font-size: 13px;
  }
