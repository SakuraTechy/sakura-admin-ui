<template>
  ...
  >
    ...
    <div class="recording-view" :class="`recording-view--${viewMode}`">
      ...
      <div v-if="form.mode !== 'createScene'" class="form-section">
        <div class="section-title">目标范围</div>
      ...
      <div v-if="form.mode === 'createScene'" class="form-section">
        <div class="section-title">新建内容</div>
        <a-row :gutter="16">
          <a-col :span="12">
      ...
      <div v-if="form.mode === 'appendCase' || form.mode === 'appendStep'" class="form-section">
        <div class="section-title">录制内容</div>
        <a-row :gutter="16">
      ...
      <a-collapse v-model:active-key="advancedActiveKeys" class="advanced-settings">
        <a-collapse-item key="advanced" header="高级选项">
          ...
          <a-row :gutter="16" class="recording-switch-row">
            <a-col :span="12">
              <a-form-item label="保存截图 artifact">
                <a-switch v-model="form.persistScreenshots" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="原始 base64 入库">
                <a-switch v-model="form.keepRawScreenshotInStep" />
              </a-form-item>
            </a-col>
          </a-row>
          <div class="recording-options">
            <div class="recording-option-group">
              <div class="recording-option-title">截图策略</div>
            ...
            <div class="recording-option-group">
              <div class="recording-option-title">执行窗口尺寸</div>
              <a-radio-group v-model="form.windowSizeMode" direction="horizontal">
                <a-radio value="maximized">默认最大化</a-radio>
                <a-radio value="current">当前窗口尺寸</a-radio>
                <a-radio value="custom">自定义尺寸</a-radio>
              </a-radio-group>
              <a-row v-if="form.windowSizeMode === 'custom'" :gutter="12" class="viewport-row">
                <a-col :span="12">
            ...
            <div v-if="viewMode === 'console'" class="recording-option-group recording-option-group--ai">
...
type RecordingViewMode = 'standard' | 'compact' | 'focus' | 'console' | 'cards' | 'split' | 'timeline'
...
watch(viewMode, (value) => {
  if (['console', 'split'].includes(value)) {
...
const resetForm = (record?: AutomationUiSceneResp | null, options: RecordingOpenOptions = {}) => {
  ...
  liveStepCount.value = 0
  recordingLog.value = null
  advancedActiveKeys.value = ['console', 'split'].includes(viewMode.value) ? ['advanced'] : []
}
