...
export function formatDuration(milliseconds) {
  // 将毫秒转换为秒
  const seconds = Math.floor(Number.parseInt(milliseconds, 10) / 1000)
  ...
  return !Number.isNaN(remainingSecondsAfterMinutes) ? formattedDuration : '-'
}
...
export function findNodeId(data, key1, value1, key2) {
  // 设置结果
  let result
  if (!data) {
  ...
  data.forEach((item, index) => {
    if (item[key1] === value1) {
      ...
      if (!result) {
        result = findNodeId(item.children, key1, value1, key2)
      } else {
        return result
      }
    }
 ...
 * 查找树结构中匹配节点的完整路径
 * @param data 树形结构数据
 * @param key1 匹配字段名（如 'key'）
 * @param value1 匹配字段值
 * @param titleKey 路径拼接字段（默认 'title'）
 * @returns {string|null} 格式如 '/层级一/层级二/层级三'
 */
export function findNodePath(data, key1, value1, titleKey = 'title') {
  const path = []

  function dfs(nodes, current) {
    for (const node of nodes) {
      if (node[key1] === value1) {
        current.push(node[titleKey])
        path.push(...current)
        return true
      }

      if (node.children?.length && Array.isArray(node.children)) {
        current.push(node[titleKey])
        if (dfs(node.children, current)) return true
        current.pop()
      }
    }
  }
